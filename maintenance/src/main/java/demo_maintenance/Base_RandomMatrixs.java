...
public class Base_RandomMatrixs {
	...
	public Base_RandomMatrixs(int stationsNum, int inputsNum, int[] buffers, double[] work_times,
	...
	public Matrix[] updateMatrix1( Matrix[] xkMatrixs, List<Machine> machines) {
		Matrix[] resultMatrixs;

		HashMap<Integer, Machine> downMachineHashMap = new HashMap<>();
		for (int i = 0; i < machines.size(); i++) {
			if (machines.get(i).isDown()) {
				downMachineHashMap.put(machines.get(i).getId(), machines.get(i));
			}
		}
		Set<Integer> integers = downMachineHashMap.keySet();

		HashMap<Integer, ArrayList<Integer>> downMachineSegmentedSequences = new HashMap<>();


		for(Integer i : integers) {
			ArrayList<Integer> SegmentedSequences= new ArrayList<>();

			for (Integer j : integers) {
				SegmentedSequences.add(downMachineHashMap.get(j).getRecover_num());
				if (j.equals(i)){
					downMachineSegmentedSequences.put(i,SegmentedSequences);
					break;
				}
			}
		}
		...
		for (Integer integer : integers) {
			Machine m = downMachineHashMap.get(integer);
			ArrayList<Integer> m_SegmentedSequences = downMachineSegmentedSequences.get(m.getId());

			int end = demand;
			for (int i = 0; i < m_SegmentedSequences.size(); i++) {

				Integer start = m_SegmentedSequences.get(i);

				double maxUpCtime = getUpStreamCtime(i,m.getId(), downMachineHashMap, xkMatrixs);

				if(maxUpCtime<0){
					maxUpCtime=0;
				}
				for (int machineStart = m.getId() - 1; machineStart < machines.size(); machineStart++) {
					for (int j = start - 1; j < end; j++) {
						xkMatrixs[machineStart].setAsDouble(xkMatrixs[machineStart].getAsDouble(0, j) + maxUpCtime, 0, j);
					}
				}
				end = start-1;
			}
		...
		resultMatrixs=xkMatrixs;
		return resultMatrixs;
	}


	private double getUpStreamCtime(int SegmentedNum,int machineId, HashMap<Integer,Machine> downMachineHashMap,Matrix[] xkMatrixs) {
		Set<Integer> integers = downMachineHashMap.keySet();
		ArrayList<Machine> downMachineList = new ArrayList<>();

		for (Integer i : integers) {
			downMachineList.add(downMachineHashMap.get(i));
		}

		double maxUpRecoverTime = (downMachineHashMap.get(machineId ).getD_time()+downMachineHashMap.get(machineId).getD_continuousTime())-
				xkMatrixs[machineId-1].getAsDouble(0,downMachineHashMap.get(machineId).getRecover_num());

		if (downMachineHashMap.size() == 1&&SegmentedNum==0) {
			return maxUpRecoverTime;
		}

		for(int i =SegmentedNum;i<downMachineList.size();i++) {

			if ((downMachineList.get(i).getD_time() + downMachineList.get(i).getD_continuousTime()) - xkMatrixs[downMachineList.get(i).getId()-1].getAsDouble(0, downMachineList.get(i).getRecover_num())
					>= maxUpRecoverTime) {
				maxUpRecoverTime = (downMachineList.get(i).getD_time() + downMachineList.get(i).getD_continuousTime()) - xkMatrixs[downMachineList.get(i).getId()-1].getAsDouble(0, downMachineList.get(i).getRecover_num());
			}
			if (downMachineList.get(i).getId() == machineId) {
				break;
			}
		}
