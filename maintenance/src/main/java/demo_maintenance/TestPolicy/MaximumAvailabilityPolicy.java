...
public class MaximumAvailabilityPolicy {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < basethroughput.length; i++) {
           data data1 = new data(Y.getAsDouble(0,i), basethroughput[i]);
            data.add(data1);
        }
        ...
        if (count>0){
            for (int y = 0; y < stationsNum; y++) {
                machines.get(y).setDown(true);
                machines.get(y).setD_time(beginTime);
                machines.get(y).setD_continuousTime(400.0);
                machines.get(y).setReliabilityFixAsNew(beginTime);
            }
        }
        for (int j = 0; j < stationsNum; j++) {
            ...
            for (double i = 0; i < beginTime; i++) {
                FailureRate failureRate = new FailureRate(i, Machinesreliability.getFailureRate(beginTime));
       ...
       Loss_JFC computeloss = new Loss_JFC(xkMatrixs, work_times, buffers, maxPluStateMatrix, machines);
        ...
        for (int i = 5334; i < 30000; i=i+1000) {
            ...
            if( 5334<i&&i <= 6734){
                for (int j = 0; j < stationsNum; j++) {
                    ...
                    ReliabilityParameters machinesreliability = machinesWithRecoverNum.get(j).getReliability();
                    double Reliability = machinesreliability.GetReliability(beginTime , beginTime+work_times[j]);
                    if (0 <= Reliability && Reliability <= 0.75) {
                        ...
                        if (count>0){
                            for (int y = 0; y < stationsNum; y++) {
                                machinesWithRecoverNum.get(y).setDown(true);
                                machinesWithRecoverNum.get(y).setD_time(beginTime);
                                machinesWithRecoverNum.get(y).setD_continuousTime(400.0);
                                machinesWithRecoverNum.get(y).setReliabilityFixAsNew(beginTime);
                            }
                        }
                for(int j=0;j<stationsNum;j++){

                    ReliabilityParameters machinesreliability = machinesWithRecoverNum.get(j).getReliability();
                    list[j].add(new FailureRate(beginTime,machinesreliability.getFailureRate(beginTime)));
                }
        ...
        mpCcompute.selectBest(beginxkMatixs, mpcComputeResult, 0.0001, 100.0);

        for (int z = 0; z < stationsNum; z++) {

           //  EasyExcel.write("F:\\data\\policydata\\SMM\\"+"设备"+(z+1)+"FailureRates.xlsx",com.Jarvis.MaxPlusMaintenance.demo_maintenance.entity.FailureRate.class).withTemplate("F:\\data\\data.xlsx").sheet("sheet1").doWrite(list[z]);
        }
      ...
      EasyExcel.write("E:\\论文草稿\\MPC\\MaximizeAvailability.xlsx",com.Jarvis.MaxPlusMaintenance.demo_maintenance.entity.data.class).withTemplate("E:\\论文草稿\\MPC\\data.xlsx").sheet("sheet1").doWrite(Finaldata);
