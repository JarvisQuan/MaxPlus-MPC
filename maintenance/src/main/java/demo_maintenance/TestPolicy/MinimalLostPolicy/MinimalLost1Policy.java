package demo_maintenance.TestPolicy.MinimalLostPolicy;
...
//产损最小策略（最小维修）
public class MinimalLost1Policy {
    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j< stationsNum; j++) {
            ...
            double Reliability = Machinesreliability.GetReliability(beginTime-work_times[j], beginTime);
            for (double i = 0; i <beginTime; i++) {
                ...
                if (0<=Reliability&&Reliability<=0.75) {
                    ...
                    machines.get(j).setD_continuousTime(200.0);
                    //根据维修策略更新可靠性模型
                    machines.get(j).setReliabilityFixAsOld(beginTime);
       ...
       Loss_JFC computeloss = new Loss_JFC(xkMatrixs, work_times, buffers, maxPluStateMatrix, machinesWithRecoverNum);
       ...
       for (int i=0; i<loss.length; i++) {
            ...
            if ( 5334<i&&i <= 6334) {
                for (int j = 0; j < stationsNum; j++) {
                    ...
                    if (0 <= v && v <= 0.75) {
                        ...
                        machinesWithRecoverNum.get(j).setD_continuousTime(200.0);

                        machinesWithRecoverNum.get(j).setReliabilityFixAsOld(beginTime);
                    }
        ...
        for (int z = 0; z < stationsNum; z++) {
             //EasyExcel.write("F:\\data\\policydata\\MinimalLoss\\"+"设备"+(z+1)+"FailureRates.xlsx",com.Jarvis.MaxPlusMaintenance.demo_maintenance.entity.FailureRate.class).withTemplate("F:\\data\\data.xlsx").sheet("sheet1").doWrite(list[z]);
        }
...
//        EasyExcel.write("E:\\论文草稿\\MPC\\MINLoss1.xlsx",com.Jarvis.MaxPlusMaintenance.demo_maintenance.entity.data.class).withTemplate("E:\\论文草稿\\MPC\\data.xlsx").sheet("sheet1").doWrite(Finaldata);
